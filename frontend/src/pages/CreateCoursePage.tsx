...
function CreateCoursePage()  {
    ...
    const [addCourse, setAddCourse] = useState("Add course to registration")
  ...
  const handleAdd = async (data: CourseData) => {

    const result = await fetch('https://10.101.128.56:6001/api/createCourse', {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    ...
    // Otherwise, load all courses
      async function loadCourses() {
        ...
        const response = await fetch('https://10.101.128.56:6001/api/coursesNotAdded');
            ...
            courses.map((course: CourseData) => (
              <div key={course.id} className = 'course-card'>
                ...
                <div className = 'card-right'>
                  ...
                  <div className='card-column'>
                    <button type="button" onClick={()=>{handleAdd(course)}}>{addCourse}</button>
                  </div>
                </div> 
              </div> /* course-card */
          ))
